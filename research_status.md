...
| 第 14 周 | 敏感性（突发长度、热点）+ 边缘队列数敏感性 | 论文第 3–5 章初稿；骨架 §5.4 定稿 | 中 | **研究日 A 已完成（2026-09-22）**：230 run（队列数 60 / 突发长度 120+30 / 热点网格 20），审计格齐。**14.2% 确认为 10 目的地挤 8 队列的混叠伪影**（10 队列下归零），第 13 周该结论与「Dynamic 端到端更优」据此收窄；三种粒度下 τ/T_burst≈1 仍为最优；长 burst 被 Dynamic 抢占截断（NoPreemption 下精确达名义值）。**研究日 B 待做**：图 8 三面板 + 抢占截断升格为 2 s 结果 |
...
Dynamic **0.00%**；发生在成束之前。**第 14 周研究日 A 已查清机制并收窄本行**：它是 `(targetLabel - 1) % numQueues` 在 10 目的地 / 8 队列下的混叠，10 队列下丢弃精确归零 ⇒ **是配置伪影，不是 Static 的性质**；「Dynamic 端到端优于 Static」随之不成立，但「核心 burst loss 与端到端不可互换」仍成立

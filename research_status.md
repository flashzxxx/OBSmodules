...
> 当前重点：有条件 Go、主叙事收缩已冻结。**第 10 周已完成（2026-09-22）：ExpA 网格已冻结**（`research_reports/2026-09-week10/网格冻结.md`）。pilot（异步臂 40 + 异步参考线 8 + 热点 6 = 54 run，全部 exit 0）给出四条结论：① **τ 最优点随偏移模型移动**——同步模型下 τ/T=1（6.78% / 13.51%），异步模型下 Horizon 移到 τ/T=2（15.28% / 23.22%），两条曲线必须分开报告；② 异步偏移下 **VF 收益 7.4–8.9 pp ≥ FDL 收益 4.9–6.4 pp**；③ **偏移模型本身的效应约 8 pp**（同一 sendInterval：同步 12.88% → 异步 20.76%），大于主角的边际效应，故"τ/T≈1 最优"**不是无条件结论**；④ **热点 ρ≈0.81 上 FDL 基本失效**——只买到 0.1–0.6 pp，而它被使用 11–16 万次（`fdlInFlightOccupancy` 最高 2.13，已打满），与低负载 6.1 pp 形成 5.1→5.5 的完整论证链。**第 9 周（2026-09-20）**：两条对照臂实现并验证——VF 调度器选项（`enableVoidFilling`，默认关，`OBS_ChannelCalendar`，Horizon 的严格推广）与 host 侧无缓存重传基线（`src/Retransmit/`，不改 `src/EdgeNode/`）；`useFDL=false` 回归**逐标量全同**（1701/1701、1697/1697）；同步偏移下 VF 补 **0 个** burst（附证明，D4 据此裁决 A+B）；重传确定性用例完成时延差 30.00 ms 恰等于超时值。第 8 周：**τ 最优区间在 τ/T_burst ≈ 1**（仅同步偏移模型），热点 **19.5 µs → ρ=0.8074**。第一篇论文骨架见 `research_reports/2026-09-week6-novelty/论文骨架.md`。**D1 已闭合（2026-09-20）**：器件参数取自公开文献与厂商资料（`research_reports/2026-09-week8/器件参数与三条不等式.md`），仅质量/体积/功耗预算留作假设 + 敏感性。第 7 周遗留的两篇单波长排队论文已于第 9 周取得**全文**（`laevens2003single`、`vanhoudt2004channel`）——**τ/T_burst≈1 属已知粒度结论，不得当新发现**。不得把第 5 周标定写成回环性能。
> 当前研究周：第 10 周已完成（2026-09-22）：**ExpA 网格已冻结**（`research_reports/2026-09-week10/网格冻结.md`），异步臂 pilot 54 run 全部 exit 0，pilot 曲线 `research_reports/figures/pilot_grid_freeze.png`。下一周为第 11 周：按冻结表跑批量（210 run，repeat 5），其中同步臂 60 run 须用**同一构建模式**重跑后再出图（第 8 周目录未记录模式）。每批前后跑 `tools/audit_runs.py <目录> --repeat 5`。
> 更新时间：2026-09-22（第 10 周收口：ExpA 网格冻结）
...
| 第 11 周 | ExpA 批量（按冻结表，210 run，repeat 5） | ExpR 批量（2 s） | 中 | 待开始 |

（第 11 周同步臂 60 run 须与异步臂**同一构建模式**重跑：第 8 周目录未记录模式，`.sca` 不能跨 debug/release 比较。）

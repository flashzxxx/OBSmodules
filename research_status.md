...
| 第 8 周 | 热点流量矩阵，确认瓶颈 ρ≥0.8 | 复标定；确认 D1 已发出（冻结日 2026-09-28） | 中 | **已完成（2026-09-18）**：瓶颈 sat3→sat2，**19.5 µs → ρ=0.8074** 且种子有差异；另提前跑通 ExpA 批量并修复 `maxFdlLoopsPerBurst` 红线配置。D1 未发出（Agent 建议发，不阻塞第 8–11 周） |
...
2. **FDL 延迟器预算（D1）。** 实际器件是由光开关组成的可调光纤延迟器，不是固定长光纤。询问稿：`research_reports/2026-09-week6-novelty/D1-询问稿.md`。回复截止 2026-09-25。**默认值冻结日 2026-09-28**：无书面回复则用 Mouammar 2026 开关表（默认电光 ≤0.1 µs；MEMS 则判 D1 失败，第一篇以无缓存+重传对照为主结果）+ 等效光纤 1 km / 0.2 dB/km / 每级插损 0.5 dB。阻塞第 10 周参数冻结。**状态：询问稿已拟，待用户发出。** 不把「已拟」写成「已发出」。

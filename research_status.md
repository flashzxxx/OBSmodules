...
> 当前重点：有条件 Go、主叙事收缩已冻结。**第 11 周已完成（2026-09-22）：225 run 批量 + 图 3–5 初版**（`research_reports/2026-09-week11/分析.md`；图在 `research_reports/figures/fig{3,4,5}_*.png`，逐格数字 `batch_summary.csv`）。四条结论：① **同步 τ 最优点 τ/T=1**（6.78% / 13.51%，与第 8 周逐位一致）、**异步 τ 最优点 τ/T=2**（15.29% / 23.22%），两条曲线必须分开；② 回环 FDL 把**端到端送达载荷**提高 **+23% / +21%**（同步、τ/T=1：2520 / 3349 Mbit/s 对无 FDL 2046 / 2777）；异步工况下 **VF 的贡献（+32% / +40%）大于延迟线本身**；③ **热点 ρ≈0.81 上 FDL 基本失效**：丢包 34.8–35.4% 与 τ 无关，而使用率 15–22%、在飞占用 0.7–2.1（τ/T=4 时 1.9–3.1，延迟线自己被塞满）；④ **第 8 周构建模式判定为 debug**（同格 50/50 全部有末位差异，平均 181 条/文件），release 重跑复现 12.88 / 18.71 / 6.78 / 13.51 **逐位一致**，故结论无需修正、但图必须单模式。**实验 R**：修掉确认包被 `minSizeWithPadding=500B` 放大 2.4 倍的建模假象（未修时不收敛，活动消息 47k→729k，32 位进程 T=1.49 s 静默 0xC0000005）；修后 79.8 µs 稳定（31k 平坦），35.6 µs 仍无界增长 ⇒ **开环重传可用区间比单级回环窄**（新发 ρ≈0.2 对 0.8），同实测负载下送达载荷仅约回环臂一半（1145 vs 2046–2520 Mbit/s），`retx/fresh ≈ 1.0`。**第 10 周（2026-09-22）**：ExpA 网格冻结（`research_reports/2026-09-week10/网格冻结.md`）。**第 9 周（2026-09-20）**：VF 调度器选项（`enableVoidFilling`，默认关，Horizon 的严格推广）与 host 侧重传基线（`src/Retransmit/`，不改 `src/EdgeNode/`）实现并验证；`useFDL=false` 回归**逐标量全同**；同步偏移下 VF 补 **0 个** burst（附证明，D4 据此裁决 A+B）。第 8 周：τ 最优 τ/T≈1（仅同步模型），热点 19.5 µs → ρ=0.8074。第一篇论文骨架见 `research_reports/2026-09-week6-novelty/论文骨架.md`。**D1 已闭合（2026-09-20）**。第 7 周遗留的两篇单波长排队论文已于第 9 周取得**全文**——**τ/T_burst≈1 属已知粒度结论，不得当新发现**。不得把第 5 周标定写成回环性能。
> 当前研究周：第 11 周已完成（2026-09-22）：225 run 批量（图 3–5 初版）。下一周为第 12 周：**图 6 可行性图**（`fdl_design.py` 接器件表，把 τ 可行区间叠到图 3 上得"有用/无增益/物理不可行"三区）+ 跑 `FDL-RateCheck` 关闭 D2；实验 R 若要进高负载区，须先做**重传窗口（限速）**的设计与确定性用例。
> 更新时间：2026-09-22（第 11 周收口：批量 + 图 3–5 初版）
...
| 阶段三实验 A/B + ExpR | 第一篇：回环工作区间、重传对照、载荷可行性、边缘是否移动交叉点 | **ExpA 批量已完成（第 11 周）**；**ExpR 已批量但只覆盖轻负载**；ExpB 待展开 | 第 11 周 225 run（release，repeat 5）：同步 τ/T=1 最优（6.78% / 13.51%）、异步 τ/T=2 最优（15.29% / 23.22%）、热点上 FDL 失效（丢包 34.8–35.4% 与 τ 无关）；端到端送达载荷回环 +21–23%、异步 VF +32–40%；重传同实测负载下只有回环一半且可用区间更窄。图 3–5 初版已出。**尚缺**：图 6 可行性图与 RateCheck（第 12 周）；ExpR 若要高负载须先做重传窗口；ExpB 依赖 Task 1.3（已完成） |
...
| 第 12 周 | 可行性图（`fdl_design.py` 接器件表，图 6 三区） | RateCheck（关闭 D2） | 中 | 待开始 |
| 第 13 周 | ExpB 2×2 批量 | 分析：联合是否移动交叉点 | 中 | 待开始 |
| 第 14 周 | 敏感性（突发长度、热点） | 论文第 3–5 章初稿 | 中 | 待开始 |
...
| 载荷可行性图与回环 vs 重传交叉点 | 阶段三实验后形成；正负结论都可写 |

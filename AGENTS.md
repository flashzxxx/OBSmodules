...
- 项目结题时间：2027 年 9 月。
- 当前重点：第 8 周已完成（2026-09-18）：ExpA 批量提前跑通（50+10 run），**τ 最优区间在 τ/T_burst ≈ 1**，ρ≈0.59 下 τ/T=4 丢包（19.61%）反高于不加 FDL（18.71%）；热点标定达成 **19.5 µs → ρ=0.8074**（瓶颈 sat3→sat2）；并修复违反"单次回环"红线的配置（`maxFdlLoopsPerBurst` 由 -1 改为 1）。研究叙事以 `research_status.md` 第十节为唯一源，不在本文件复述。**下一周为第 9 周**：实现 VF 调度器选项与 host 侧重传基线（均默认关）+ 两者确定性用例；取得 `laevens2003single` / `vanhoudt2004channel` 全文或摘要（排队论目前仅题录级）。第一篇论文骨架：`research_reports/2026-09-week6-novelty/论文骨架.md`。**D1 已于 2026-09-20 闭合**：不再向他人询问，改为从公开文献与厂商资料建器件参数表（`research_reports/2026-09-week8/器件参数与三条不等式.md`），仅质量/体积/功耗预算留作假设 + 敏感性。不得把第 5 周标定（ρ=0.4→35.9 µs，`useFDL=false`）写成回环性能；**第 7 周重标定为 35.6 µs，35.9 µs 仅属第 5 周记录**。Task 2.1–2.3 三分支与 ICMP 回归已通过；pre-FDL 对照有条件通过（仅 `my ID` 不同）；release 链接（`-linet`）已由用户解决。

...
- 项目结题时间：2027 年 9 月。
- 当前重点：**第 9 周已完成（2026-09-20），D4 已裁决为 A+B 并行**。第 9 周交付两条对照臂——VF 调度器选项（`enableVoidFilling`，默认关，`src/CoreNode/OBS_ChannelCalendar.*`，是 Horizon 的严格推广）与 host 侧无缓存重传基线（`src/Retransmit/`，不改 `src/EdgeNode/`）；`useFDL=false` 回归**逐标量全同**（Basic/TooShort/Off/SO 1701/1701、Compatibility 1697/1697）。最重要的结论是负面的：同步偏移（各源同 `maxOffset`）下 VF 补了 **0 个** burst——到达顺序 ≡ BCP 顺序，被 horizon 拒绝者必与已有预约窗口重叠，**同步情形无空隙可补**（附证明）；**D4 裁决：主网格 `ExpA-TauSweep`/`ExpA-NoFDL` 保持第 8 周同步偏移 + Horizon 口径不变，把"VF≡Horizon"写成结构性结论，另建异步偏移臂 `FDL-Scenario-Async` + `ExpA-AsyncTauSweep`/`ExpA-AsyncNoFDL` 回答"何时 VF 才值"**（0.2 s 冒烟：VF 把丢包降 7.9–9.4 个百分点）。第 8 周结论不变：ExpA 批量 50+10 run，**τ 最优区间在 τ/T_burst ≈ 1**，ρ≈0.59 下 τ/T=4 丢包（19.61%）反高于不加 FDL（18.71%）；热点标定 **19.5 µs → ρ=0.8074**（瓶颈 sat3→sat2）。研究叙事以 `research_status.md` 第十节为唯一源，不在本文件复述。**下一周为第 10 周**：ExpA pilot 冻结网格（异步臂最终 repeat、是否并入 ρ≈0.81 热点矩阵需"流量形态"维度）；冻结时把构建模式写进结果目录名（`.sca` 不能跨 debug/release 比较）。第一篇论文骨架：`research_reports/2026-09-week6-novelty/论文骨架.md`。**D1 已于 2026-09-20 闭合**：器件参数取自公开文献与厂商资料（`research_reports/2026-09-week8/器件参数与三条不等式.md`），仅质量/体积/功耗预算留作假设 + 敏感性。第 7 周遗留的两篇单波长排队论文已于第 9 周取得**全文**（`laevens2003single`、`vanhoudt2004channel`）——**τ/T_burst≈1 属已知粒度结论，不得当新发现**。不得把第 5 周标定（ρ=0.4→35.9 µs，`useFDL=false`）写成回环性能；**第 7 周重标定为 35.6 µs，35.9 µs 仅属第 5 周记录**。Task 2.1–2.3 三分支与 ICMP 回归已通过；pre-FDL 对照有条件通过（仅 `my ID` 不同）；release 链接（`-linet`）已由用户解决。

...
- 项目结题时间：2027 年 9 月。
- 当前重点：**第 11 周已完成（2026-09-22）：225 run 批量 + 图 3–5 初版**（`research_reports/2026-09-week11/分析.md`；图在 `research_reports/figures/fig{3,4,5}_*.png`，逐格数字 `batch_summary.csv`）。结论：① **同步 τ 最优点 τ/T=1**（6.78% / 13.51%）、**异步最优点 τ/T=2**（15.29% / 23.22%），两条曲线必须分开；② 回环 FDL 把**端到端送达载荷**提高 **+23% / +21%**（2520 / 3349 对无 FDL 2046 / 2777 Mbit/s），异步工况下 **VF 的贡献（+32% / +40%）大于延迟线本身**；③ **热点 ρ≈0.81 上 FDL 基本失效**（丢包 34.8–35.4% 与 τ 无关；在飞占用 τ/T=4 时 1.9–3.1，延迟线自己被塞满）；④ **第 8 周判定为 debug 构建**（同格 50/50 全部末位差异，平均 181 条/文件），release 重跑复现 12.88 / 18.71 / 6.78 / 13.51 **逐位一致**。**实验 R**：修掉 64 B 确认被 `minSizeWithPadding=500B` 放大 2.4 倍的建模假象（未修则不收敛，活动消息 47k→729k、32 位进程静默 0xC0000005）；修后开环重传可用区间只到新发 ρ≈0.2（35.6 µs 无界增长），同实测负载下送达载荷仅约回环一半（1145 对 2046–2520），`retx/fresh ≈ 1.0`。研究叙事以 `research_status.md` 第十节为唯一源，不在本文件复述。**下一周为第 12 周**：图 6 可行性图（`fdl_design.py` 接器件表，得"有用/无增益/物理不可行"三区）+ 跑 `FDL-RateCheck` 关闭 D2；实验 R 若要进高负载区须先做**重传窗口（限速）**。**不得**把同步与异步 τ 曲线合成一条；**不得**把重传臂的"请求送达比例"与回环臂的"包送达比例"直接相减；**不得**用新发负载轴代替实测负载轴比较两条臂。第 10 周：ExpA 网格冻结（`research_reports/2026-09-week10/网格冻结.md`）。第 9 周：VF 调度器选项（`enableVoidFilling`，默认关，`src/CoreNode/OBS_ChannelCalendar.*`，Horizon 的严格推广）与 host 侧无缓存重传基线（`src/Retransmit/`，不改 `src/EdgeNode/`）；`useFDL=false` 回归**逐标量全同**；同步偏移下 VF 补 0 个 burst（附证明，D4 据此裁决 A+B）。第一篇论文骨架：`research_reports/2026-09-week6-novelty/论文骨架.md`。**D1 已于 2026-09-20 闭合**：器件参数取自公开文献与厂商资料（`research_reports/2026-09-week8/器件参数与三条不等式.md`）。第 7 周遗留的两篇单波长排队论文已于第 9 周取得**全文**——**τ/T_burst≈1 属已知粒度结论，不得当新发现**。不得把第 5 周标定（ρ=0.4→35.9 µs，`useFDL=false`）写成回环性能；**第 7 周重标定为 35.6 µs，35.9 µs 仅属第 5 周记录**。Task 2.1–2.3 三分支与 ICMP 回归已通过；pre-FDL 对照有条件通过（仅 `my ID` 不同）；release 链接（`-linet`）已由用户解决。

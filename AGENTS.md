...
- 项目结题时间：2027 年 9 月。
- 当前重点：第 9 周已完成（2026-09-20）：实现并验证两条对照臂——VF 调度器选项（`enableVoidFilling`，默认关，`src/CoreNode/OBS_ChannelCalendar.*`，是 Horizon 的严格推广）与 host 侧无缓存重传基线（`src/Retransmit/`，不改 `src/EdgeNode/`）；`useFDL=false` 回归**逐标量全同**（Basic/TooShort/Off/SO 1701/1701、Compatibility 1697/1697）。第 9 周最重要的结论是负面的：载入实测下 VF 补了 **0 个** burst——`OBS_BurstSender` 使所有 burst 偏移恒等于 `maxOffset`（1 ms），到达顺序 ≡ BCP 顺序，被 horizon 拒绝者必与已有预约窗口重叠，**同步偏移的 W=1 链路无可补空隙**（附证明）；把各源偏移改成 700–970 µs 后 VF 立即生效（全网已调度 +23.0%）。第 8 周结论不变：ExpA 批量 50+10 run，**τ 最优区间在 τ/T_burst ≈ 1**，ρ≈0.59 下 τ/T=4 丢包（19.61%）反高于不加 FDL（18.71%）；热点标定 **19.5 µs → ρ=0.8074**（瓶颈 sat3→sat2）。研究叙事以 `research_status.md` 第十节为唯一源，不在本文件复述。**下一周为第 10 周**：ExpA pilot（3 负载 × 调度器 × 5 τ）并冻结网格；**先决项 D4 待用户裁决**——VF 在同步偏移下恒等于 Horizon，是加"偏移异质"维度（采纳则 ExpA 需重跑）还是把 VF 降级为结构结论；冻结网格时把构建模式写进结果目录名（`.sca` 不能跨 debug/release 比较）。第一篇论文骨架：`research_reports/2026-09-week6-novelty/论文骨架.md`。**D1 已于 2026-09-20 闭合**：器件参数取自公开文献与厂商资料（`research_reports/2026-09-week8/器件参数与三条不等式.md`），仅质量/体积/功耗预算留作假设 + 敏感性。第 7 周遗留的两篇单波长排队论文已于第 9 周取得**全文**（`laevens2003single`、`vanhoudt2004channel`）——**τ/T_burst≈1 属已知粒度结论，不得当新发现**。不得把第 5 周标定（ρ=0.4→35.9 µs，`useFDL=false`）写成回环性能；**第 7 周重标定为 35.6 µs，35.9 µs 仅属第 5 周记录**。Task 2.1–2.3 三分支与 ICMP 回归已通过；pre-FDL 对照有条件通过（仅 `my ID` 不同）；release 链接（`-linet`）已由用户解决。

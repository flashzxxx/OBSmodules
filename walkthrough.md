...
这次没有把工作区一次性提交，而是按改动所属的实现阶段拆开，便于将来按阶段回退：

| 提交 | 作者日期 | 内容 |
|---|---|---|
| `c95fb3b` | 2026-09-18 | `test:` FDL 场景/测试/实验配置（`omnetpp.ini` + 3 个 `fdl_*.ini`，4 文件） |
| `f2c523e` | 2026-09-15 | `feat:` FDL 审核项落地（11 个 `src/**` 文件） |
| `1de832f` | 2026-09-17 | `fix:` 崩溃修复（17 文件：9 个构造函数置 NULL、`lambdasPerPort` 上界与计数校验、2 个析构守卫） |
| `0c80eff` | 2026-09-20 | `docs:` 第 1–8 周报告、文献台账、架构图、结果图 |
| `e05839c` | 2026-09-20 | `chore:` 结果不入库、忽略本地工具、移除 `.download-ripgrep/tmp-file` |

**拆法**：`src/**` 的改动横跨两个阶段——09-15 的审核项落地与 09-17 由 M1 复跑暴露的崩溃修复——
其中 6 个文件（`OBS_CoreControlLogic`、`OBS_CoreOutputHorizon`、`OBS_OpticalCrossConnect` 的
`.cc/.h`）同时含两个阶段的改动，因此做了**行级**拆分：先按 09-17 的改动清单移除对应代码块并提交
09-15 状态，再恢复并提交 09-17 的块。拆分前对全部 27 个待提交文件记录 SHA256，恢复后逐字节一致
（0 处不匹配）；`1de832f` 的 diff 恰为 **17 文件 +133/−10**，与事后统计完全吻合。

**注意**：中间的 `f2c523e` 未重新编译验证（Agent 不跑 make）。它是 09-15 当时的真实状态，
而 `research_reports/2026-09-code-review/walkthrough.md` 记录该状态于 09-17 由用户确认编译通过。
提交身份沿用仓库既有作者 `zhengzhx <759274219@qq.com>`，通过环境变量传入，**未写入任何 git config**；
作者日期按各阶段设定，提交日期为实际提交时间。
